...
export class CommentController {
  constructor(private readonly commentService: CommentService, private readonly commentQueryRepository: CommentQueryRepositoryMongodb) {}

  @UseGuards(GetUserIdFromBearerToken)
  @Get(':commentId')
  @HttpCode(200)
  async findCommentById(@Param('commentId') commentId: string, @UserId() userId: string | null): Promise<CommentViewModel> {
  ...
  @Put(':commentId')
  @HttpCode(204)
  async updateCommentById(@Param('commentId') commentId: string, @User() user: UserEntity, @Body() updateCommentDto: UpdateCommentDto) {
    return this.commentService.updateCommentById(commentId, user.id, updateCommentDto.content);
  }
  ...
  async changeReactionForComment(
    ...
    return this.commentService.changeReactionForComment(commentId, user.id, user.accountData.login, likeStatusDto.likeStatus);
  }

  @UseGuards(BearerAuthGuard)
  @Delete(':commentId')
  @HttpCode(204)
  async deleteCommentById(@Param('commentId') commentId: string, @User() user: UserEntity) {
